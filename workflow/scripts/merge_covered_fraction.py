...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Merge per-sample CoverM covered-fraction values"
    )
...
def load_covered_fraction(file, sample_name):
    ...
    fraction_column = find_metric_column(columns, "coveredfraction")
    if not contig_column or not fraction_column:
        raise ValueError(
            f"CoverM file {file} must contain Contig and Covered Fraction columns"
        )

    output = dataframe[[contig_column, fraction_column]].copy()
...
def merge(files, sample_names):
    ...
    merged = load_covered_fraction(files[0], sample_names[0])
    for file, sample_name in zip(files[1:], sample_names[1:]):
        merged = merged.join(
            load_covered_fraction(file, sample_name),
            how="outer",
        )
